...
class AIBot:
    ...
    def setup_handlers(self):
        ...
        # --- ترحيب المجموعات (طريقة مضمونة) ---
        @self.dp.message(F.new_chat_members)
        async def on_added_to_group(message: Message):
            bot_obj = await self.bot.get_me()
            for member in message.new_chat_members:
                if member.id == bot_obj.id:
                    welcome_text = (
                        f"👋 **أهلاً بكم! أنا {BOT_NAME}**\n\n"
                        f"أنا أعمل بالذكاء الاصطناعي لمساعدتكم.\n"
                        f"نادني بكلمة '**{BOT_NAME}**' أو '**بوت**' وسأرد عليك فوراً.\n"
                        f"استخدم `/draw` للرسم الذكي.\n\n"
                        f"👤 المطور: {DEVELOPER_USER}"
                    )
                    await message.answer(welcome_text, parse_mode="Markdown")
                    
                    # تنبيه المطور
                    alert = f"🔔 **تفعيل جديد!**\n🏷️ المجموعة: {message.chat.title}\n🆔 الايدي: `{message.chat.id}`"
                    await self.bot.send_message(DEVELOPER_ID, alert, parse_mode="Markdown")

        # --- معالجة الأزرار (التعليمات) ---
        @self.dp.callback_query(F.data == "how_to_use")
        async def cb_help(callback: CallbackQuery):
            help_text = (
                f"📖 **دليل استخدام {BOT_NAME}:**\n\n"
                f"1️⃣ في الخاص: أرسل أي سؤال مباشرة.\n"
                f"2️⃣ في المجموعات: ابدأ رسالتك بكلمة '{BOT_NAME}' أو 'بوت'.\n"
                f"3️⃣ للرسم: استخدم أمر `/draw` متبوعاً بوصف الصورة.\n"
                f"4️⃣ المطور: اسألني 'من هو مطورك' وسأجيبك."
            )
        ...
        async def cb_draw(callback: CallbackQuery):
            await callback.message.answer("🎨 أرسل `/draw` متبوعاً بوصف الصورة بالإنجليزية.")
            await callback.answer()
        ...
        async def cmd_draw(message: Message):
            ...
            if not prompt:
                return await message.reply(f"⚠️ يرجى كتابة وصف للصورة. مثال: `/draw {BOT_NAME} in space`", parse_mode="Markdown")
        ...
        async def chat_handler(message: Message):
            ...
            # الرد على الأسئلة عن المطور (نظام ثابت)
            dev_keywords = ["مطور", "المطور", "علي حسين", "من صنعك", "صاحب البوت"]
