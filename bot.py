        ...
        return InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [
                    InlineKeyboardButton(
                        text="📢 مشاركة العملاق",
                        switch_inline_query="جرب بوت العملاق الذكي!"
                    )
                ]
            ]
        )
    ...
    def setup_handlers(self):
        ...
        async def cmd_start(message: Message):
            ...
            welcome_text = (
                ...
                f"أنا بوت ذكاء اصطناعي متطور أعمل بمحركات **Gemini** و **SDXL**.\n"
        ...
        async def cmd_draw(message: Message):
            ...
            if not prompt:
                await message.reply(
                    f"⚠️ يرجى كتابة وصف للصورة.\n\nمثال:\n`/draw {BOT_NAME} في الفضاء`",
                    parse_mode="Markdown"
                )
                return
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            await message.reply(
                                "❌ **عذراً، خدمة توليد الصور مشغولة حالياً.**"
                            )
        ...
        async def chat_handler(message: Message):
            ...
            is_asking_about_dev = any(
                word in text for word in dev_keywords
            )
            ...
            if is_private or mentions_name or mentions_bot or is_reply_to_bot:
                ...
                # إذا المستخدم فقط نادى البوت
                if not user_input and (mentions_name or mentions_bot):

                    await message.reply(
                        f"نعم يا غالي، **{BOT_NAME}** معك! 🤖"
                    )
                ...
                try:
                    ...
                    prompt = f"""
أنت مساعد ذكي واسمك هو {BOT_NAME}.

المطور الخاص بك هو {DEVELOPER_NAME} وحسابه {DEVELOPER_USER}.

أجب بأسلوب فخم وذكي باللغة العربية.

إذا سألك أحد عن المطور فاذكر اسمه وحسابه.
...
"""
                    ...
                    if not reply_text:
                        reply_text = "⚠️ لم أستطع إنشاء رد حالياً."
                    ...
                    await message.reply(
                        "⚠️ عذراً، حدث خطأ أثناء التفكير. حاول مرة أخرى!"
                    )
        ...
        async def cb_help(callback: types.CallbackQuery):
            ...
            help_text = (
                ...
                f"• في الخاص: أرسل أي سؤال مباشرة.\n"
                f"• في المجموعات:\n"
                f"- ابدأ رسالتك بكلمة '{BOT_NAME}'\n"
                f"- أو كلمة 'بوت'\n"
                f"- أو قم بالرد على إحدى رسائلي\n\n"
                f"🎨 لتوليد صورة:\n"
                f"`/draw وصف الصورة`"
            )
        ...
        async def cb_draw(callback: types.CallbackQuery):
            ...
            await callback.message.answer(
                "🎨 أرسل الأمر التالي مع وصف الصورة:\n\n`/draw قطة في الفضاء`",
                parse_mode="Markdown"
            )
...
if __name__ == "__main__":
    ...
    if not TELEGRAM_TOKEN or not GEMINI_API_KEY:
        ...
        print("❌ خطأ: تأكد من إضافة المتغيرات البيئية!")
