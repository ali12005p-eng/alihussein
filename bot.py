...
class AIBot:
    ...
    def setup_handlers(self):
        ...
        async def chat_handler(message: Message):
            ...
            # التحقق من المناداة بـ "العملاق" أو "بوت"
            ...
            if is_private or mentions_name or mentions_bot or is_reply_to_bot:
                ...
                # تنظيف النص من الكلمات المنادية
                user_input = text
                if mentions_name:
                ...
                # إذا ناداه فقط بدون نص
                if not user_input and (mentions_name or mentions_bot):
                    await message.reply(f"نعم يا غالي، **{BOT_NAME}** معك! كيف يمكنني مساعدتك اليوم؟")
                    return
                ...
                try:
                    chat_completion = self.groq_client.chat.completions.create(
                        messages=[
                            {
                                "role": "system", 
                                "content": f"أنت مساعد ذكي واسمك هو {BOT_NAME}. المطور الخاص بك هو {DEVELOPER_NAME} ({DEVELOPER_USER}). "
                                           f"أنت الآن تتفاعل في مجموعة تيليجرام، اجعل ردودك فخمة، ذكية، وتليق باسمك '{BOT_NAME}'."
                            },
        ...
        async def cb_chat(callback: types.CallbackQuery):
            await callback.message.answer(f"🤖 **{BOT_NAME}** جاهز لخدمتك! تفضل بإرسال سؤالك.")
            await callback.answer()
